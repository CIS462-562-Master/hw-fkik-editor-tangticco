    ...
    case YXZ:
        ...
        if (angleRad[VX] > -M_PI_2 + EPSILON)
        {
            ...
            else
            {
                /*WARNING.  Not a unique solution.*/
                angleRad[VZ] = 0.0f;
                angleRad[VY] = atan2(mM[2][1], mM[0][0]);
            }
        }
        else
        {
...
void quat::FromRotation(const mat3& rot)
{
    ...
    {
        float S = sqrt(1 + rot[0][0] - rot[1][1] - rot[2][2]) * 2.0;
    ...
    {
        float S = sqrt(1 + rot[1][1] - rot[0][0] - rot[2][2]) * 2.0;
    ...
    {
        float S = sqrt(1 + rot[2][2] - rot[1][1] - rot[0][0]) * 2.0;
        mQ[VW] = (rot[1][0] - rot[0][1]) / S;
        mQ[VX] = (rot[0][2] + rot[2][0]) / S;
        mQ[VY] = (rot[1][2] + rot[2][1]) / S;
        mQ[VZ] = 0.25 * S;
    }
    //help from class slides and https://www.euclideanspace.com/maths/geometry/rotations/conversions/matrixToQuaternion/
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
    ...
    float angle = acos(Dot(q0, q1));
    quat q = (sin(angle * (1 - u)) / sin(angle)) * q0 + (sin(angle * u) / sin(angle)) * q1;
...
quat quat::SBisect(const quat& a, const quat& b)
{
    ...
    quat q = (a + b) / sqrt(Dot(a, a) + Dot(b, b));
...
void quat::FromAxisAngle(const vec3& axis, double angleRad)
{
    ...
    mQ[VX] = sin(angleRad / 2) * (axis[VX] / sqrt((axis[VX] * axis[VX] + axis[VY] * axis[VY] + axis[VZ] * axis[VZ])));
    mQ[VY] = sin(angleRad / 2) * (axis[VY] / sqrt((axis[VX] * axis[VX] + axis[VY] * axis[VY] + axis[VZ] * axis[VZ])));
    mQ[VZ] = sin(angleRad / 2) * (axis[VZ] / sqrt((axis[VX] * axis[VX] + axis[VY] * axis[VY] + axis[VZ] * axis[VZ])));
}
