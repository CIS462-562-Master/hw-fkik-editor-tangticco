...
void generateControlPoints(quat& q_1, quat& q0, quat& q1, quat& q2, std::vector<quat>& mCtrlPoints) {
	quat b0, b1, b2, b3;
	quat  q1_prime, q1_star, q0_right_prime, q0_right_star;
	b0 = q0;
	q1_prime = quat::SDouble(q_1, q0);
	q1_star = quat::SBisect(q1_prime, q1);
	b1 = quat::Slerp(q0, q1_star, (1.0 / 3.0));
	q0_right_prime = quat::SDouble(q2, q1);
	q0_right_star = quat::SBisect(q0, q0_right_prime);
	b2 = quat::Slerp(q1, q0_right_star, (1.0 / 3.0));
	b3 = q1;
...
void ASplineQuat::computeControlPoints(quat& startQuat, quat& endQuat)
{
	...
	generateControlPoints(startQuat, mKeys[0].second, mKeys[1].second, mKeys[2].second, mCtrlPoints);
	for (int segment = 1; segment < numKeys - 2; segment++)
	{
		...
		generateControlPoints(mKeys[segment - 1].second, mKeys[segment].second, mKeys[segment + 1].second, mKeys[segment + 2].second, mCtrlPoints);
	}
	generateControlPoints(mKeys[numKeys - 3].second, mKeys[numKeys - 2].second, mKeys[numKeys - 1].second, endQuat, mCtrlPoints);
